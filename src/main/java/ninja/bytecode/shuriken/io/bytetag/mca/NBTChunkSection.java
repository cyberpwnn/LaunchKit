...
public class NBTChunkSection {
	...
	private NBTListTag<NBTCompoundTag> palette;
	private byte[] blockLight;
	private long[] blockStates;
	private byte[] skyLight;
	private int dataVersion;

	public NBTChunkSection(NBTCompoundTag sectionRoot, int dataVersion) {
		this(sectionRoot, dataVersion, MCALoadFlags.ALL_DATA);
	}

	public NBTChunkSection(NBTCompoundTag sectionRoot, int dataVersion, long loadFlags) {
		data = sectionRoot;
		this.dataVersion = dataVersion;
		NBTListTag<?> rawPalette = sectionRoot.getListTag("Palette");
		if (rawPalette == null) {
			return;
		}
		palette = rawPalette.asCompoundTagList();
		for (int i = 0; i < palette.size(); i++) {
			NBTCompoundTag data = palette.get(i);
			putValueIndexedPalette(data, i);
		}

		NBTByteArrayTag blockLight = sectionRoot.getByteArrayTag("BlockLight");
		NBTLongArrayTag blockStates = sectionRoot.getLongArrayTag("BlockStates");
		NBTByteArrayTag skyLight = sectionRoot.getByteArrayTag("SkyLight");

		if ((loadFlags & MCALoadFlags.BLOCK_LIGHTS) != 0) {
			this.blockLight = blockLight != null ? blockLight.getValue() : null;
		}
		if ((loadFlags & MCALoadFlags.BLOCK_STATES) != 0) {
			this.blockStates = blockStates != null ? blockStates.getValue() : null;
		}
		if ((loadFlags & MCALoadFlags.SKY_LIGHT) != 0) {
	...
	void putValueIndexedPalette(NBTCompoundTag data, int index) {
	...
	PaletteIndex getValueIndexedPalette(NBTCompoundTag data) {
	...
	private static class PaletteIndex {
		...
		PaletteIndex(NBTCompoundTag data, int index) {
			this.data = data;
			this.index = index;
		}
	}
	 ...
	 */
	public NBTListTag<NBTCompoundTag> getPalette() {
		return palette;
	}

	int addToPalette(NBTCompoundTag data) {
		PaletteIndex index;
	 ...
	 */
	public static NBTChunkSection newSection() {
		NBTChunkSection s = new NBTChunkSection();
		s.blockStates = new long[256];
		s.palette = new NBTListTag<>(NBTCompoundTag.class);
		NBTCompoundTag air = new NBTCompoundTag();
		air.putString("Name", "minecraft:air");
		s.palette.add(air);
		s.data = new NBTCompoundTag();
		return s;
	}
	 ...
	 */
	public NBTCompoundTag updateHandle(int y) {
