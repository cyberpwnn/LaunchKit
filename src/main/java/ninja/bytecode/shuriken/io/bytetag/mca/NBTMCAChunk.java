...
public class NBTMCAChunk {
	...
	private int[] biomes;
	private NBTCompoundTag heightMaps;
	private NBTCompoundTag carvingMasks;
	private NBTChunkSection[] sections = new NBTChunkSection[16]; //always initialized with size = 16 for fast access
	private NBTListTag<NBTCompoundTag> entities;
	private NBTListTag<NBTCompoundTag> tileEntities;
	private NBTListTag<NBTCompoundTag> tileTicks;
	private NBTListTag<NBTCompoundTag> liquidTicks;
	private NBTListTag<NBTListTag<?>> lights;
	private NBTListTag<NBTListTag<?>> liquidsToBeTicked;
	private NBTListTag<NBTListTag<?>> toBeTicked;
	private NBTListTag<NBTListTag<?>> postProcessing;
	private String status;
	private NBTCompoundTag structures;

	NBTMCAChunk(int lastMCAUpdate) {
	 ...
	 */
	public NBTMCAChunk(NBTCompoundTag data) {
		this.data = data;
	...
	private void initReferences(long loadFlags) {
		...
		if ((loadFlags & (BLOCK_LIGHTS|BLOCK_STATES|SKY_LIGHT)) != 0 && level.containsKey("Sections")) {
			for (NBTCompoundTag section : level.getListTag("Sections").asCompoundTagList()) {
				int sectionIndex = section.getByte("Y");
				if (sectionIndex > 15 || sectionIndex < 0) {
					continue;
				}
				NBTChunkSection newSection = new NBTChunkSection(section, dataVersion, loadFlags);
	...
	public int serialize(RandomAccessFile raf, int xPos, int zPos) throws IOException {
		...
		try (BufferedOutputStream nbtOut = new BufferedOutputStream(MCACompressionType.ZLIB.compress(baos))) {
			new NBTSerializer(false).toStream(new NBTRawNamedTag(null, updateHandle(xPos, zPos)), nbtOut);
		}
		byte[] rawData = baos.toByteArray();
		raf.writeInt(rawData.length + 1); // including the byte to store the compression type
		raf.writeByte(MCACompressionType.ZLIB.getID());
		raf.write(rawData);
	 ...
	 * Reads chunk data from a RandomAccessFile. The RandomAccessFile must already be at the correct position.
	 * @param raf The RandomAccessFile to read the chunk data from.
	 * @param loadFlags A logical or of {@link MCALoadFlags} constants indicating what data should be loaded
	 * @throws IOException When something went wrong during reading.
	 */
	public void deserialize(RandomAccessFile raf, long loadFlags) throws IOException {
		byte compressionTypeByte = raf.readByte();
		MCACompressionType compressionType = MCACompressionType.getFromID(compressionTypeByte);
		...
		NBTRawNamedTag tag = new NBTDeserializer(false).fromStream(dis);
		if (tag != null && tag.getTag() instanceof NBTCompoundTag) {
			data = (NBTCompoundTag) tag.getTag();
	...
	public NBTCompoundTag getBlockStateAt(int blockX, int blockY, int blockZ) {
		NBTChunkSection section = sections[MCAUtil.blockToChunk(blockY)];
		if (section == null) {
			return null;
		}
	 ...
	 */
	public void setBlockStateAt(int blockX, int blockY, int blockZ, NBTCompoundTag state, boolean cleanup) {
		int sectionIndex = MCAUtil.blockToChunk(blockY);
		NBTChunkSection section = sections[sectionIndex];
		if (section == null) {
			section = sections[sectionIndex] = NBTChunkSection.newSection();
		}
	 ...
	 * @return The Section.
	 */
	public NBTChunkSection getSection(int sectionY) {
	 ...
	 */
	public void setSection(int sectionY, NBTChunkSection section) {
	 ...
	 */
	public void setHeightMaps(NBTCompoundTag heightMaps) {
	 ...
	 */
	public NBTCompoundTag getCarvingMasks() {
		return carvingMasks;
	}
	 ...
	 */
	public void setCarvingMasks(NBTCompoundTag carvingMasks) {
	 ...
	 */
	public NBTListTag<NBTCompoundTag> getEntities() {
		return entities;
	}
	 ...
	 */
	public void setEntities(NBTListTag<NBTCompoundTag> entities) {
	 ...
	 */
	public NBTListTag<NBTCompoundTag> getTileEntities() {
		return tileEntities;
	}
	 ...
	 */
	public void setTileEntities(NBTListTag<NBTCompoundTag> tileEntities) {
	 ...
	 */
	public NBTListTag<NBTCompoundTag> getTileTicks() {
		return tileTicks;
	}
	 ...
	 */
	public void setTileTicks(NBTListTag<NBTCompoundTag> tileTicks) {
	 ...
	 */
	public NBTListTag<NBTCompoundTag> getLiquidTicks() {
		return liquidTicks;
	}
	 ...
	 */
	public void setLiquidTicks(NBTListTag<NBTCompoundTag> liquidTicks) {
	 ...
	 */
	public NBTListTag<NBTListTag<?>> getLights() {
		return lights;
	}
	 ...
	 */
	public void setLights(NBTListTag<NBTListTag<?>> lights) {
		this.lights = lights;
	}
	 ...
	 */
	public NBTListTag<NBTListTag<?>> getLiquidsToBeTicked() {
	 ...
	 */
	public void setLiquidsToBeTicked(NBTListTag<NBTListTag<?>> liquidsToBeTicked) {
	 ...
	 */
	public NBTListTag<NBTListTag<?>> getToBeTicked() {
		return toBeTicked;
	}
	 ...
	 */
	public void setToBeTicked(NBTListTag<NBTListTag<?>> toBeTicked) {
	 ...
	 */
	public NBTListTag<NBTListTag<?>> getPostProcessing() {
	 ...
	 */
	public void setPostProcessing(NBTListTag<NBTListTag<?>> postProcessing) {
	 ...
	 */
	public NBTCompoundTag getStructures() {
		return structures;
	}
	 ...
	 */
	public void setStructures(NBTCompoundTag structures) {
	...
	public void cleanupPalettesAndBlockStates() {
		for (NBTChunkSection section : sections) {
			if (section != null) {
	...
	public static NBTMCAChunk newChunk() {
		NBTMCAChunk c = new NBTMCAChunk(0);
		c.dataVersion = DEFAULT_DATA_VERSION;
		c.data = new NBTCompoundTag();
		c.data.put("Level", new NBTCompoundTag());
		c.status = "mobs_spawned";
		return c;
	}

	public NBTCompoundTag updateHandle(int xPos, int zPos) {
		data.putInt("DataVersion", dataVersion);
		NBTCompoundTag level = data.getCompoundTag("Level");
		...
		NBTListTag<NBTCompoundTag> sections = new NBTListTag<>(NBTCompoundTag.class);
