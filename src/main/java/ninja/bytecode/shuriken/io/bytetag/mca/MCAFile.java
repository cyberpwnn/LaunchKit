...
public class MCAFile {
	...
	public void deserialize(RandomAccessFile raf) throws IOException {
		deserialize(raf, MCALoadFlags.ALL_DATA);
	}
	...
	public void deserialize(RandomAccessFile raf, long loadFlags) throws IOException {
		...
		for (int i = 0; i < 1024; i++) {
			...
			NBTMCAChunk chunk = new NBTMCAChunk(timestamp);
	...
	public int serialize(RandomAccessFile raf, boolean changeLastUpdate) throws IOException {
		...
		for (int cx = 0; cx < 32; cx++) {
			for (int cz = 0; cz < 32; cz++) {
				...
				NBTMCAChunk chunk = chunks[index];
				if (chunk == null) {
					continue;
				}
	 ...
	 */
	public void setChunk(int index, NBTMCAChunk chunk) {
		checkIndex(index);
		if (chunks == null) {
			chunks = new NBTMCAChunk[1024];
		}
	 ...
	 * @param chunk The chunk to be set.
	 */
	public void setChunk(int chunkX, int chunkZ, NBTMCAChunk chunk) {
	 ...
	 * */
	public NBTMCAChunk getChunk(int index) {
		checkIndex(index);
		if (chunks == null) {
			return null;
		}
	 ...
	 * @return The chunk data.
	 * */
	public NBTMCAChunk getChunk(int chunkX, int chunkZ) {
	...
	private NBTMCAChunk createChunkIfMissing(int blockX, int blockZ) {
		int chunkX = MCAUtil.blockToChunk(blockX), chunkZ = MCAUtil.blockToChunk(blockZ);
		NBTMCAChunk chunk = getChunk(chunkX, chunkZ);
		if (chunk == null) {
			chunk = NBTMCAChunk.newChunk();
			setChunk(getChunkIndex(chunkX, chunkZ), chunk);
		}
		return chunk;
	}
	...
	public int getBiomeAt(int blockX, int blockZ) {
		int chunkX = MCAUtil.blockToChunk(blockX), chunkZ = MCAUtil.blockToChunk(blockZ);
		NBTMCAChunk chunk = getChunk(getChunkIndex(chunkX, chunkZ));
		if (chunk == null) {
			return -1;
		}
